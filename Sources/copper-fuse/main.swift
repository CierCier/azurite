...
@main
struct CopperFuseMount: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "copper-fuse",
    ...
    @Option(name: .shortAndLong, help: "Mount point directory")
    ...
    func run() throws {
        ...
        let absoluteArchiveURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent(archivePath).standardized
        ...
        let mPoint = mountPoint ?? default_mount(archiveURL)

        let mountURL = URL(fileURLWithPath: mPoint).standardized
        ...
        let encryptionKey = try key.map {
            try CopperEncryptionAlgorithm.deriveKey(from: $0, salt: salt)
        }

        // Open archive
        // Print("Opening archive \(finalArchiveURL.path)...")
        let archive = try CopperArchive.open(
            path: finalArchiveURL.path, encryptionKey: encryptionKey)
        ...
        ops.create = { (path, mode, fi) -> Int32 in
            ...
            if !FileManager.default.createFile(
                atPath: tempFile.path, contents: nil, attributes: [.posixPermissions: mode])
            {
            ...
            if (flags & O_ACCMODE) == O_WRONLY || (flags & O_ACCMODE) == O_RDWR {
                ...
                if FileManager.default.fileExists(atPath: tempFile.path) {
                    ...
                    do {
                        ...
                        let newEntry = try context.withArchive { archive -> CopperFileEntry? in
                            ...
                            if let index = archive.findFileEntry(filename: name) {
                                return archive.fileEntries[index]
                            }
                            return nil
                        }
    ...
    func default_mount(_ archiveURL: URL) -> String {
        #if os(macOS)
            return "/Volumes/\(archiveURL.lastPathComponent)"
        #elseif os(Linux)
            let uid = getuid()
            let path = "/run/user/\(uid)/copper/\(archiveURL.lastPathComponent)"
            return path
        #else
            // probably windows
